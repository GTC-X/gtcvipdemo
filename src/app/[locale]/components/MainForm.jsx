  ...
  const sendDataToDb = async (data) => {
    ...
    try {
      ...
      if (mt5Res?.data?.success) {
        toast.success(mt5Res?.data?.message);
        ...
        const mailRes = await axios.post(`/api/mt5-completion-mail`, {
          name: data.nickname,
          phone: data.phone,
          email: data.email,
          password: data.password,
        ...
        toast.error(mt5Res?.data?.message || t("submitError"));
      }
    } catch (err) {
        ...
        <div className="grid grid-cols-1 gap-3 mb-3">
          <div className="relative">
            <div className="relative">
              ...
              <div
                className={`absolute top-9 bg-primary ltr:right-3 rtl:left-3 rounded-md text-white py-1.5 px-2 ${
                  formik.touched.email && formik.errors.email
